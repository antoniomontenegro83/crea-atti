...
const generator = {
    ...
    // ─────────────────────────────────────────────
    //  CONVERTITORE NUMERO → LETTERE ITALIANE
    //  es. "1200,25"  → "milleduecento/25"
    //      "1000000,54" → "unmilione/54"
    // ─────────────────────────────────────────────
    _numToItalian: (n) => {
        if (n === 0) return 'zero';
        ...
        if (n < 100) {
            const ten  = t[Math.floor(n / 10)];
            const unit = u[n % 10];
            // Elisione: ventuno, ventotto (non ventiuno, ventotto)
            if (unit === 'uno' || unit === 'otto') return ten.slice(0, -1) + unit;
            return ten + unit;
        }
    ...
    // Converte stringa importo "1.200,25" o "1200,25" → "milleduecento/25"
    euroToLetters: (str) => {
        if (!str) return '';
        // Normalizza: rimuove separatori migliaia (. o spazio), virgola → punto decimale
        const clean = str.toString().trim().replace(/\./g,'').replace(/\s/g,'').replace(',','.');
        const parts = clean.split('.');
        const intVal  = parseInt(parts[0], 10);
        const centStr = parts[1] ? parts[1].substring(0, 2).padEnd(2, '0') : null;
        if (isNaN(intVal)) return '';
        const letters = generator._numToItalian(intVal);
        return centStr ? `${letters}/${centStr}` : letters;
    },

    // Rileva e aggiorna tutti i campi _in_lettere_ dal loro campo sorgente
    autoFillLetters: () => {
        document.querySelectorAll('#form-container input[name*="_in_lettere"]').forEach(dest => {
            // Trova il campo sorgente: rimuove "_in_lettere" dal nome
            const srcName = dest.name.replace('_in_lettere', '');
            const src = document.querySelector(`#form-container input[name="${srcName}"]`);
            if (!src) return;
            const converted = generator.euroToLetters(src.value);
            if (converted) {
                dest.value = converted;
                dest.style.background = '#f0fdf4';
                dest.style.color      = '#15803d';
            } else {
                dest.value = '';
            ...
            <div id="tab-panel-compile" class="flex-1 flex gap-6 overflow-hidden">
                ...
                <div class="flex-[1.2] flex flex-col gap-5 overflow-hidden min-w-[240px]">
                    <div class="bg-white p-4 rounded-[2rem] border shadow-sm flex-1 flex flex-col overflow-hidden">
                        ...
                        <!-- scroll container: il sizer tiene le dimensioni reali scaled -->
    ...
    init: () => {
        ...
        if (!core.db.contacts) core.db.contacts = {};

        const numEl = document.getElementById('gen-act-number');
        if (numEl) numEl.value = (core.db.lastActNumber || 0) + 1;

        generator.renderSlots();
        generator.renderFolderSelects();
        generator.renderTplList();
        generator.renderContacts();
    ...
    prepareForm: () => {
        ...
        allTags.forEach(tag=>{
            ...
            const isLetters=/_in_lettere/i.test(tag);  // auto-calcolato
            const div=document.createElement('div');
            div.className=isWide?'col-span-3':'col-span-1';
            let inputHTML;
            if(isLetters){
                // Campo sola lettura — auto compilato dal campo importo corrispondente
                inputHTML=`<div class="relative"><input type="text" name="${tag}" class="gen-input ${colClass}" value="${oldValues[tag]||''}" readonly style="background:#f0fdf4;color:#15803d;cursor:default;border-color:#bbf7d0;padding-right:50px;" placeholder="auto…" title="Compilato automaticamente dal campo importo"><span style="position:absolute;right:10px;top:50%;transform:translateY(-50%);font-size:10px;font-weight:900;color:#16a34a;text-transform:uppercase;letter-spacing:0.05em;">auto</span></div>`;
            } else if(isDate){
                ...
                inputHTML=`<div class="relative"><input type="text" name="${tag}" class="gen-input ${colClass}" value="${oldValues[tag]||''}" onfocus="core.state.currentInput=this; generator.showSuggestions(this,'${tag}')" oninput="generator.updateLivePreview(); generator.showSuggestions(this,'${tag}')" autocomplete="off" placeholder="..."><div class="autocomplete-dropdown hidden" data-for="${tag}"></div></div>`;
            }
    ...
    //  AUTOCOMPLETE
    // ─────────────────────────────────────────────
    showSuggestions: (input, tag) => {
        const dropdown=input.parentElement?.querySelector('.autocomplete-dropdown');
        if(!dropdown) return;
        const q=input.value.toLowerCase();
        const contacts=Object.values(core.db.contacts||{});
        if(!contacts.length||!q){dropdown.classList.add('hidden');return;}
        const matches=contacts.filter(c=>c.name?.toLowerCase().includes(q)||c.cf?.toLowerCase().includes(q)).slice(0,5);
        if(!matches.length){dropdown.classList.add('hidden');return;}
        dropdown.innerHTML=matches.map(c=>`<div onclick="generator.selectContact(this,'${tag}')" data-name="${c.name}" data-cf="${c.cf||''}" data-address="${c.address||''}" class="px-4 py-2.5 cursor-pointer hover:bg-blue-50 font-bold text-[13px] text-slate-700 border-b border-slate-50 last:border-0"><span class="block">${c.name}</span><span class="text-[11px] text-slate-400 font-medium">${c.cf||''}</span></div>`).join('');
        dropdown.classList.remove('hidden');
    },

    selectContact: (item, tag) => {
        const input=item.closest('.relative')?.querySelector('input[type="text"]');
        if(!input) return;
        input.value=item.dataset.name||'';
        const cfEl=document.querySelector('#form-container input[name*="CF"],#form-container input[name*="CODICE"]');
        const addrEl=document.querySelector('#form-container input[name*="INDIR"],#form-container input[name*="RESIDEN"]');
        if(cfEl&&!cfEl.value)   cfEl.value=item.dataset.cf||'';
        if(addrEl&&!addrEl.value) addrEl.value=item.dataset.address||'';
        item.closest('.autocomplete-dropdown')?.classList.add('hidden');
        generator.updateLivePreview();
    },

    // ─────────────────────────────────────────────
    //  ZOOM — aggiorna sizer, scroll funziona correttamente
    // ─────────────────────────────────────────────
    setZoom: (val) => {
        ...
        // Sizer = dimensioni fisiche × zoom → lo scroll le "vede" correttamente
        sz.style.width  = (794 * z) + 'px';
        sz.style.height = (lp.scrollHeight * z) + 'px';
        if (lbl) lbl.innerText = generator._previewZoom + '%';
    },

    // ─────────────────────────────────────────────
    //  ANTEPRIMA LIVE — fedele al Word
    // ─────────────────────────────────────────────
    updateLivePreview: () => {
        // Prima aggiorna tutti i campi _in_lettere_
        ...
        // Sostituisce {{tag}} e {tag} solo nei nodi testo, non negli attributi HTML
        const fill = (html) => html.replace(/(<[^>]+>)|([^<]+)/g, (m, tag, txt) => {
            ...
            return txt.replace(/\{\{([^}]+)\}\}|\{([^}]+)\}/g, (t, a, b) => {
                ...
                const val = data[key];
                return val && val.trim()
                    ? `<span class="pv-filled">${val.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</span>`
                    : `<span class="pv-tag">${t}</span>`;
            });
        });

        if (tpl.bodyHtml) {
            lp.innerHTML = css + fill(tpl.bodyHtml);
        } else if (tpl.body) {
            ...
            const rendered = esc.replace(/\{\{([^}]+)\}\}|\{([^}]+)\}/g, (t, a, b) => {
                ...
                const val = data[key];
                return val && val.trim()
                    ? `<span class="pv-filled">${val}</span>`
                    : `<span class="pv-tag">${t}</span>`;
            });
            lp.innerHTML = css + `<div style="white-space:pre-wrap;">${rendered}</div>`;
        } else {
            lp.innerHTML = '<div style="color:#cbd5e1;font-size:10pt;padding:20px 0;">Anteprima non disponibile</div>';
        }
    ...
    submit: async (event) => {
        ...
        const actNum=document.getElementById('gen-act-number')?.value||'1';
        data['NUMERO_ATTO']=actNum;
        ...
        for(const item of sel){
            try{
                ...
                const filename=`${safeName}_n${actNum}_${today.replace(/\//g,'-')}.docx`;
        ...
        if(generated.length>0){
            core.db.lastActNumber=parseInt(actNum);
            core.save();
            const nextEl=document.getElementById('gen-act-number');
            if(nextEl) nextEl.value=parseInt(actNum)+1;
  ...
  * { font-family: 'Inter', serif; box-sizing: border-box; }
  ...
  @media print {
    ...
    saveContact: () => {
        ...
        if(!core.db.contacts) core.db.contacts={};
        core.db.contacts['ct_'+Date.now()]={
            name,
            cf:document.getElementById('ct-cf')?.value.trim(),
        ...
        core.save();
        ['ct-name','ct-cf','ct-address','ct-phone','ct-email'].forEach(id=>{const el=document.getElementById(id);if(el) el.value='';});
        generator.renderContacts();
        core.showToast(`"${name}" aggiunto alla rubrica!`);
    },
    ...
    renderContacts: () => {
        ...
        const filtered=Object.entries(core.db.contacts||{}).filter(([,c])=>!q||c.name?.toLowerCase().includes(q)||c.cf?.toLowerCase().includes(q)||c.type?.toLowerCase().includes(q));
        if(countEl) countEl.innerText=`${filtered.length} contatti`;
        if(!filtered.length){grid.innerHTML=`<div class="col-span-2 py-20 text-center opacity-40"><i class="fas fa-address-book text-5xl text-slate-300 mb-4 block"></i><p class="font-black uppercase text-slate-400 text-[13px]">Nessun contatto</p></div>`;return;}
        const colors={'Cliente':'blue','Controparte':'red','Perito / CTU':'purple','Altro':'slate'};
        grid.innerHTML=filtered.map(([id,c])=>{
            const col=colors[c.type]||'slate';
            const phoneHtml=c.phone?`<a href="tel:${c.phone}" class="text-[11px] text-slate-400 hover:text-blue-500 font-bold"><i class="fas fa-phone mr-1"></i>${c.phone}</a>`:'';
            const emailHtml=c.email?`<a href="mailto:${c.email}" class="text-[11px] text-slate-400 hover:text-blue-500 font-bold truncate"><i class="fas fa-envelope mr-1"></i>${c.email}</a>`:'';
            const cfHtml=c.cf?`<p class="text-[11px] text-slate-400 font-mono">${c.cf}</p>`:'';
            return `<div class="contact-card group"><div class="flex justify-between items-start mb-3"><span class="text-[10px] font-black uppercase bg-${col}-50 text-${col}-500 px-2.5 py-1 rounded-lg">${c.type||'Contatto'}</span><button onclick="generator.deleteContact('${id}')" class="text-slate-200 hover:text-red-500 transition-all opacity-0 group-hover:opacity-100"><i class="fas fa-trash text-xs"></i></button></div><h5 class="font-black text-[14px] text-slate-800 uppercase leading-tight mb-1">${c.name}</h5>${cfHtml}<div class="flex flex-wrap gap-3 mt-3 pt-3 border-t border-slate-50">${phoneHtml}${emailHtml}</div></div>`;
        }).join('');
    },

    deleteContact: (id) => {
        if(!confirm("Eliminare questo contatto?")) return;
        delete core.db.contacts[id];
        core.save();
        generator.renderContacts();
        core.showToast("Contatto eliminato.");
    }
};