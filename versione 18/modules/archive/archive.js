...
const archive = {
    ...
    autoFillForm: (numero, annoFascicolo) => {
        ...
        const f = Object.values(core.arcDb).find(f => {
            const sameNum = String(f.numero || '').trim() === String(numero).trim();
            if (!sameNum) return false;
            if (targetYear) return (f.year || archive._extractYear(f.numero)) === targetYear;
            return true;
        });
        if (!f) return;

        // Compila i campi del form per nome parziale
        ...
        fill(['oggetto'],              f.title);
        fill(['capitolo', 'articolo'], f.capitolo);
        fill(['uep'],                  f.uep);
        fill(['operatore'],            f.operatore);
        fill(['anno_fascicolo'],       String(f.year || ''));
    ...
    saveToDisk: async (folderId, blobs) => {
        ...
        try {
            ...
            return true;/* --- modules/archive/archive.js --- */
const archive = {
    selectedFolder: null,
    ...
    autoFillForm: (numero, annoFascicolo) => {
        ...
        const f = Object.values(core.arcDb).find(rec => {
            const sameNum = String(rec.numero || '').trim() === String(numero).trim();
            if (!sameNum) return false;
        ...
        // Mappa nome campo ESATTO nel template → valore dal fascicolo
        const exactMap = {
            'oggetto'        : f.title,
            'capitolo'       : f.capitolo,
            'articolo'       : f.capitolo,   // stesso valore di capitolo
            'uep'            : f.uep,
            'operatore'      : f.operatore,
            'anno_fascicolo' : String(f.year || ''),
        };

        Object.entries(exactMap).forEach(([fieldName, value]) => {
            if (!value) return;
            // Cerca case-insensitive
            document.querySelectorAll('#form-container input[name], #form-container textarea[name]').forEach(el => {
                if (el.name.toLowerCase() === fieldName.toLowerCase()) {
                    el.value = value;
                }
            });
        });
