    ...
    prepareForm: () => {
        ...
        allTags.forEach(tag=>{
            ...
            if(isLetters){
                ...
                const onchange = (tag.toLowerCase() === 'anno_fascicolo')
                    ? ` onchange="archive.autoFillForm(document.querySelector('#form-container input[name=Fascicolo], #form-container input[name=fascicolo]')?.value, this.value)"`
                    : '';
                inputHTML=`<div class="relative"><input type="text" name="${tag}" class="gen-input ${colClass}" value="${oldValues[tag]||''}" onfocus="core.state.currentInput=this" oninput="generator.updateLivePreview()"${onchange} placeholder="..."></div>`;
            }
        ...
        // ── Auto-fill da fascicolo ──────────────────────────────────────────
        // Quando l'utente digita nel campo fascicolo, cerca in arcDb e
        // compila automaticamente anno_fascicolo, capitolo, uep, operatore
        const fascEl = fd.querySelector('input[name="fascicolo"], input[name="Fascicolo"], input[name="FASCICOLO"]');
        if (fascEl) {
            fascEl.addEventListener('change', () => {
                generator._autoFillFromFascicolo(fascEl.value.trim());
            });
        }
    },

    _autoFillFromFascicolo: (fascNum) => {
        if (!fascNum || !core.arcDb) return;

        // Legge anno_fascicolo dal form se già compilato
        const annoEl = document.querySelector('#form-container input[name="anno_fascicolo"], #form-container input[name="Anno_Fascicolo"], #form-container input[name="ANNO_FASCICOLO"]');
        const fascYear = annoEl ? parseInt(annoEl.value) : null;

        // Cerca fascicolo in arcDb per numero (e anno se disponibile)
        const match = Object.values(core.arcDb).find(f => {
            const sameNum = String(f.numero || '').trim().toLowerCase() === fascNum.toLowerCase();
            if (!sameNum) return false;
            if (fascYear) return (f.year || archive._extractYear(f.numero)) === fascYear;
            return true;
        });
        if (!match) return;

        // Mappa: nome campo form → valore dal fascicolo
        const autoMap = [
            { names: ['anno_fascicolo','Anno_Fascicolo','ANNO_FASCICOLO'], value: match.year },
            { names: ['capitolo','Capitolo','CAPITOLO','capitolo_articolo','Capitolo_Articolo'], value: match.capitolo },
            { names: ['uep','UEP','Uep'], value: match.uep },
            { names: ['operatore','Operatore','OPERATORE'], value: match.operatore },
        ];

        let filled = false;
        autoMap.forEach(({ names, value }) => {
            if (!value) return;
            names.forEach(name => {
                const el = document.querySelector(`#form-container input[name="${name}"], #form-container textarea[name="${name}"]`);
                if (el && !el.value) { el.value = value; filled = true; }
            });
        });

        // Compila anche per corrispondenza parziale
        document.querySelectorAll('#form-container input[name], #form-container textarea[name]').forEach(el => {
            const nm = el.name.toLowerCase();
            if (!el.value) {
                if (nm.includes('capitolo') || nm.includes('articolo')) el.value = match.capitolo || el.value;
                else if (nm.includes('uep'))       el.value = match.uep       || el.value;
                else if (nm.includes('operatore')) el.value = match.operatore || el.value;
            }
        });

        if (filled) generator.updateLivePreview();
    },
