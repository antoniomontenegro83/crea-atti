...
const archive = {
    ...
    openFolder: (id) => {
        ...
        // Leggi dal form Crea Atti cercando per nome parziale
        const _fromForm = (keywords) => {
            let val = '';
            document.querySelectorAll('#form-container input[name], #form-container textarea[name]').forEach(el => {
                if (val) return;
                const nm = el.name.toLowerCase();
                if (keywords.some(k => nm.includes(k)) && el.value.trim()) val = el.value.trim();
            });
            return val;
        };

        // Aggiorna arcDb con i dati del form se mancanti, poi salva
        let changed = false;
        if (!f.title || f.title.trim() === String(f.numero).trim()) {
            const v = _fromForm(['oggetto']); if (v) { f.title = v; changed = true; }
        }
        if (!f.capitolo) { const v = _fromForm(['capitolo','articolo']); if (v) { f.capitolo = v; changed = true; } }
        if (!f.uep)      { const v = _fromForm(['uep']);                 if (v) { f.uep      = v; changed = true; } }
        if (!f.operatore){ const v = _fromForm(['operatore']);           if (v) { f.operatore= v; changed = true; } }
        if (changed) core.saveArc();
    ...
    // Cerca fascicolo per numero e compila i campi del form automaticamente
    autoFillForm: (numero, annoFascicolo) => {
        if (!numero || !core.arcDb) return;
        const targetYear = annoFascicolo ? parseInt(annoFascicolo) : null;

        // Cerca per numero e anno
        const f = Object.values(core.arcDb).find(rec => {
            const sameNum = String(rec.numero || '').trim().toLowerCase() === String(numero).trim().toLowerCase();
            if (!sameNum) return false;
            if (targetYear) return (rec.year || archive._extractYear(rec.numero)) === targetYear;
            return true;
        });
        if (!f) return;

        // 1. Prima compila dai campi principali del fascicolo
        const fillMap = [
            { keywords: ['oggetto'],              value: f.title     },
            { keywords: ['capitolo', 'articolo'], value: f.capitolo  },
            { keywords: ['uep'],                  value: f.uep       },
            { keywords: ['operatore'],             value: f.operatore },
            { keywords: ['anno_fascicolo'],        value: String(f.year || '') },
        ];

        document.querySelectorAll('#form-container input[name], #form-container textarea[name]').forEach(el => {
            const nm = el.name.toLowerCase();
            fillMap.forEach(({ keywords, value }) => {
                if (!value) return;
                if (keywords.some(k => nm.includes(k))) el.value = value;
            });
        });

        // 2. Poi compila dal backup formData (tutti gli altri campi)
        if (f.formData) {
            document.querySelectorAll('#form-container input[name], #form-container textarea[name]').forEach(el => {
                // Non sovrascrivere fascicolo e anno che l'utente ha già scritto
                const nm = el.name.toLowerCase();
                if (nm.includes('fascicolo') || nm.includes('anno_fasc')) return;
                const saved = f.formData[el.name];
