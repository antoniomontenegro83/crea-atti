...
const archive = {
    ...
    renderFolders: () => {
        ...
        grid.innerHTML = entries.map(([id, folder]) => {
            ...
            <div onclick="archive.openFolder('${id}')"
                ...
                <p class="text-blue-600 font-black text-[18px] leading-tight relative z-10">${archive._fascLabel(folder.rg, folder.year) || '—'}</p>
    ...
    saveFolder: async () => {
        ...
        core.arcDb[id] = {
            ...
            capitolo : document.getElementById('new-folder-capitolo').value.trim(),
            uep      : document.getElementById('new-folder-uep').value.trim(),
            operatore: document.getElementById('new-folder-operatore').value.trim(),
