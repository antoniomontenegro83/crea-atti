    ...
    html: `
        <div class="h-full flex flex-col overflow-hidden">
            ...
            <div class="flex gap-1 bg-slate-100 p-1 rounded-2xl mb-5 shrink-0 self-start">
                <button onclick="generator.tab('compile')"  id="tab-compile"  class="gen-tab active px-7 py-3 rounded-xl font-black uppercase text-[14px] tracking-widest transition-all"><i class="fas fa-file-signature mr-2"></i>Compila Atto</button>
                <button onclick="generator.tab('editor')"   id="tab-editor"   class="gen-tab px-7 py-3 rounded-xl font-black uppercase text-[14px] tracking-widest transition-all"><i class="fas fa-pencil-alt mr-2"></i>Editor Template</button>
                <button onclick="generator.tab('firma')"    id="tab-firma"    class="gen-tab px-7 py-3 rounded-xl font-black uppercase text-[14px] tracking-widest transition-all"><i class="fas fa-signature mr-2"></i>Firma / Timbro</button>
            </div>
                ...
                <div class="flex-[2] flex flex-col gap-5 overflow-hidden">
                    ...
                    <div class="bg-white p-7 rounded-[2rem] border shadow-sm flex-1 flex flex-col overflow-hidden">
                        <div class="flex justify-between items-center mb-5 shrink-0 flex-wrap gap-3">
                            ...
                            <div id="gen-folder-select-wrap" class="flex items-center gap-2">
                                <label class="text-[11px] font-black uppercase text-slate-400">Fascicolo</label>
                                <select id="gen-folder-select" onchange="generator.onFolderChange()"
                                    class="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 font-bold text-[12px] text-slate-700 outline-none focus:border-blue-400 max-w-[220px]">
...
<!-- ══════════════ TAB: FIRMA / TIMBRO ══════════════ -->
    ...
    //  GENERA PDF DA DOCX
    // ─────────────────────────────────────────────
    _docxToPdf: async (docxBlob, data, tpl) => {
        ...
        try {
            ...
            // Fallback: usa il body del template con tag sostituiti
            html = tpl.bodyHtml || tpl.body || '';
        }

        if (!html.trim()) return null;

        // 2. Crea un iframe nascosto per renderizzare l'HTML
        ...
        iDoc.write(`<!DOCTYPE html><html><head><meta charset="utf-8">
            <style>
                body { font-family: Calibri, Arial, sans-serif; font-size: 11pt; margin: 2.54cm 3.17cm; color: #000; line-height: 1.4; }
                p { margin: 0 0 6pt 0; }
