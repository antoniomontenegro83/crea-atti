...
Object.assign(generator, {
    ...
    onFolderChange: () => {
        ...
        // Mappa: lista di possibili nomi campo → valore da fascicolo
        // Ogni entry: { patterns: [...], value, skipPattern }
        const autoMap = [
            { patterns: ['fascicolo'],                          value: f.numero,    skip: 'anno' },
            { patterns: ['anno_fasc', 'anno_fascicolo'],        value: String(f.year || '') },
            { patterns: ['oggetto'],                            value: f.title    },
            { patterns: ['capitolo', 'articolo', 'cap_art'],    value: f.capitolo },
            { patterns: ['uep'],                                value: f.uep      },
            { patterns: ['operatore'],                          value: f.operatore},
        ];

        document.querySelectorAll('#form-container input[name], #form-container textarea[name]').forEach(el => {
            const nm = el.name.toLowerCase().replace(/[^a-z0-9]/g, '_');
            autoMap.forEach(({ patterns, value, skip }) => {
                if (!value || !value.toString().trim()) return;
                if (skip && nm.includes(skip)) return;
                const matches = patterns.some(p => nm.includes(p));
                if (matches) el.value = value; // sovrascrive sempre
            });
        });
