...
Object.assign(generator, {
    ...
    updateLivePreview: () => {
        ...
        const replaceInHtml = (html) => {
            // Regex: {{...}} o {...} dove il contenuto può contenere tag HTML
            // Cattura tutto tra le graffe, poi fa strip dei tag per ricavare il nome
            return html.replace(/\{\{((?:[^{}]|<[^>]+>)+?)\}\}/g, (m, d1) => {
                const raw  = d1;
                // Rimuove eventuali tag HTML dentro il nome e normalizza
                const key  = generator._normKey(raw.replace(/<[^>]+>/g, ''));
                const val  = data[key] ?? data[key.replace(/_/g,' ')];
                if (val && val.trim() !== '') {
                    return `<span class="preview-filled">${val.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</span>`;
                } else {
                    // Mostra il nome pulito senza tag HTML interni
                    const cleanName = raw.replace(/<[^>]+>/g,'').trim();
                    return `<span class="preview-tag">{${cleanName}}</span>`;
                }
            });
        };
