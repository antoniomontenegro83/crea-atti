...
const archive = {
    ...
    addDocToFolder: (folderId, docRecord) => {
        if (!folderId || !core.db.arc[folderId]) return false;
    ...
    // Trova fascicolo per numero RG oppure lo crea automaticamente
    findOrCreateByRg: (rg, titleFallback) => {
        if (!rg) return null;
        if (!core.db.arc) core.db.arc = {};

        // Cerca esistente (RG o titolo)
        const existing = Object.keys(core.db.arc).find(id => {
            const f = core.db.arc[id];
            return f.rg === rg || String(f.rg||'').toLowerCase() === rg.toLowerCase();
        });
    ...
    // Salva i blob fisicamente su disco in Fascicoli/{rg}/
    // blobs = { filename: Blob, ... }
    saveToDisk: async (folderId, blobs) => {
        if (!core.dirHandle) { core.showToast('Apri prima la cartella di lavoro.'); return false; }
        const f = core.db.arc[folderId];
        if (!f) return false;
        const folderName = (f.rg || f.title || folderId).replace(/[/\\:*?"<>|]/g, '_');
        try {
            const fascicoliDir = await core.dirHandle.getDirectoryHandle('Fascicoli', { create: true });
            const fascDir      = await fascicoliDir.getDirectoryHandle(folderName, { create: true });
            ...
            core.showToast('Errore salvataggio su disco: ' + err.message);
            return false;
        }
    }
};