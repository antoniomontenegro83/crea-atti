...
Object.assign(generator, {
    ...
    updateDatePreview: () => {
        ...
        const preview=inLetters?generator.dateToLetters(d):`${dd}/${mm}/${d.getFullYear()}`;
        document.getElementById('date-letters-preview').innerText=preview;
    },
    addDays: (n) => {
    ...
    confirmDate: () => {
        ...
        // Salva sempre in formato gg/mm/aaaa — la checkbox "in lettere" lo converte al volo
        const formatted=inLetters?generator.dateToLetters(d):`${dd}/${mm}/${yyyy}`;
        const input=document.querySelector(`#form-container input[name="${generator._currentDateField}"]`);
        if(input){
            input.value=formatted;
            // Se in lettere, salva anche rawDate per il nome file
            if(inLetters){
                input.dataset.rawDate=`${dd}/${mm}/${yyyy}`;
                input.readOnly=true;
    ...
    // Checkbox "in lettere" sui campi data
    // checked=true  → converte "31/12/2026" in "31 dicembre 2026" (read-only)
    // checked=false → ripristina il valore numerico originale (editabile)
    toggleDateLetters: (tag, checked) => {
        const inp = document.getElementById(`input-${tag}`);
        if (!inp) return;

        if (checked) {
            // Salva il valore numerico originale come attributo
            inp.dataset.rawDate = inp.value;

            // Converti gg/mm/aaaa → "31 dicembre 2026"
            const parts = inp.value.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/);
            if (parts) {
                const d = new Date(`${parts[3]}-${parts[2].padStart(2,'0')}-${parts[1].padStart(2,'0')}T00:00:00`);
                const months = ['gennaio','febbraio','marzo','aprile','maggio','giugno',
                                'luglio','agosto','settembre','ottobre','novembre','dicembre'];
                inp.value = `${d.getDate()} ${months[d.getMonth()]} ${d.getFullYear()}`;
            }
            inp.readOnly = true;
            inp.style.background = '#f0fdf4';
            inp.style.color = '#15803d';
        } else {
            // Ripristina valore numerico originale
            if (inp.dataset.rawDate) inp.value = inp.dataset.rawDate;
            inp.readOnly = false;
