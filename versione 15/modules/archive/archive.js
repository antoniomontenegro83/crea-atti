...
const archive = {
    ...
    renderYearTabs: () => {
        ...
        // Sincronizza dropdown filter-anno con _selectedYear
        const annoEl = document.getElementById('filter-anno');
        if (annoEl) annoEl.value = archive._selectedYear ? String(archive._selectedYear) : '';
    ...
    _onCapitoloFilter: () => {
        // Reset uep e operatore quando cambia capitolo
        const u = document.getElementById('filter-uep');
        const o = document.getElementById('filter-operatore');
        if (u) u.value = '';
        if (o) o.value = '';
    ...
    _onUepFilter: () => {
        // Reset operatore quando cambia uep
        const o = document.getElementById('filter-operatore');
        if (o) o.value = '';
        archive.renderFolders();
    },
    ...
    _populateFilters: (yearFilteredEntries) => {
        ...
        const fill = (id, values, placeholder) => {
            ...
            el.innerHTML = `<option value="">${placeholder}</option>` +
                sorted.map(v => `<option value="${v}" ${current === v ? 'selected' : ''}>${v}</option>`).join('');
            if (current && !sorted.includes(current)) el.value = '';
        };

        // Anno: sempre tutti gli anni disponibili
        const fillAnno = document.getElementById('filter-anno');
        if (fillAnno) {
            const currentAnno = fillAnno.value;
            ...
            fillAnno.innerHTML = `<option value="">Tutti gli Anni</option>` +
                years.map(y => `<option value="${y}" ${String(y) === currentAnno ? 'selected' : ''}>${y}</option>`).join('');
        }

        // Capitolo: basato sull'anno
        fill('filter-capitolo', pool.map(f => f.capitolo), 'Tutti i Capitoli');

        // UEP: basato su anno + capitolo selezionato
        const capVal = document.getElementById('filter-capitolo')?.value || '';
        const poolUep = capVal ? pool.filter(f => f.capitolo === capVal) : pool;
        fill('filter-uep', poolUep.map(f => f.uep), 'Tutti gli UEP');

        // Operatore: basato su anno + capitolo + UEP selezionati
        const uepVal = document.getElementById('filter-uep')?.value || '';
        const poolOp = uepVal ? poolUep.filter(f => f.uep === uepVal) : poolUep;
        fill('filter-operatore', poolOp.map(f => f.operatore), 'Tutti gli Operatori');
    },
    ...
    renderFolders: () => {
        ...
        const hasFilter = query || capFilter || uepFilter || opFilter || archive._selectedYear;
    ...
    findOrCreate: (numero, titleFallback, yearOverride) => {
        ...
        const existing = Object.keys(core.arcDb).find(id => {
            const f = core.arcDb[id];
            return String(f.numero || '').trim().toLowerCase() === numero.toLowerCase();
        });
